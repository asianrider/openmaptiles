...
RETURNS TABLE(osm_id bigint, geometry geometry, name text, name_en text, name_de text, name_fr text, name_it text, name_es text, name_nl text, name_ru text, tags hstore, place city_place, "rank" int, capital int) AS $$
    ...
    COALESCE(NULLIF(name_fr, ''), name_en, name) AS name_fr,
    COALESCE(NULLIF(name_it, ''), name_en, name) AS name_it,
    COALESCE(NULLIF(name_es, ''), name_en, name) AS name_es,
    COALESCE(NULLIF(name_nl, ''), name_en, name) AS name_nl,
    COALESCE(NULLIF(name_ru, ''), name_en, name) AS name_ru,
    tags,
