...
CREATE MATERIALIZED VIEW osm_important_waterway_linestring AS (
    SELECT
        ...
        name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags
    FROM (
        SELECT
            ST_LineMerge(ST_Union(geometry)) AS geometry,
            name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, slice_language_tags(tags) AS tags
        FROM osm_waterway_linestring
        WHERE name <> '' AND waterway = 'river'
        GROUP BY name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, slice_language_tags(tags)
    ) AS waterway_union
);
...
CREATE MATERIALIZED VIEW osm_important_waterway_linestring_gen1 AS (
    SELECT ST_Simplify(geometry, 60) AS geometry, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags
