...
CREATE OR REPLACE VIEW waterway_z3 AS (
    SELECT geometry, 'river'::text AS class, NULL::text AS name, NULL::text AS name_en, NULL::text AS name_de,
      NULL::text AS name_fr, NULL::text AS name_it, NULL::text AS name_es, NULL::text AS name_nl, NULL::text AS name_ru,
      NULL::hstore AS tags
...
CREATE OR REPLACE VIEW waterway_z9 AS (
    SELECT geometry, 'river'::text AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags FROM osm_important_waterway_linestring_gen3
);

-- etldoc: osm_important_waterway_linestring_gen2 ->  waterway_z10
CREATE OR REPLACE VIEW waterway_z10 AS (
    SELECT geometry, 'river'::text AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags FROM osm_important_waterway_linestring_gen2
);

-- etldoc:osm_important_waterway_linestring_gen1 ->  waterway_z11
CREATE OR REPLACE VIEW waterway_z11 AS (
    SELECT geometry, 'river'::text AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags FROM osm_important_waterway_linestring_gen1
);

-- etldoc: osm_waterway_linestring ->  waterway_z12
CREATE OR REPLACE VIEW waterway_z12 AS (
    SELECT geometry, waterway AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags FROM osm_waterway_linestring
    WHERE waterway IN ('river', 'canal')
);

-- etldoc: osm_waterway_linestring ->  waterway_z13
CREATE OR REPLACE VIEW waterway_z13 AS (
    SELECT geometry, waterway::text AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags FROM osm_waterway_linestring
...
RETURNS TABLE(geometry geometry, class text, name text, name_en text, name_de text,
  name_fr text, name_it text, name_es text, name_nl text, name_ru text, tags hstore) AS $$
