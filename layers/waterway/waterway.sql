...
CREATE OR REPLACE VIEW waterway_z3 AS (
    SELECT geometry, 'river'::text AS class, NULL::text AS name, NULL::text AS name_en, NULL::text AS name_de,
      ...
      NULL::hstore AS tags, NULL::boolean AS is_bridge, NULL::boolean AS is_tunnel, NULL::boolean AS is_intermittent
...
CREATE OR REPLACE VIEW waterway_z9 AS (
    SELECT geometry, 'river'::text AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags, NULL::boolean AS is_bridge, NULL::boolean AS is_tunnel, NULL::boolean AS is_intermittent 
...
CREATE OR REPLACE VIEW waterway_z12 AS (
    SELECT geometry, waterway AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags, NULL::boolean AS is_bridge, NULL::boolean AS is_tunnel, NULL::boolean AS is_intermittent
...
CREATE OR REPLACE VIEW waterway_z13 AS (
    SELECT geometry, waterway::text AS class, name, name_en, name_de, name_fr, name_it, name_es, name_nl, name_ru, tags, NULL::boolean AS is_bridge, NULL::boolean AS is_tunnel, NULL::boolean AS is_intermittent
    FROM osm_waterway_linestring
...
RETURNS TABLE(geometry geometry, class text, name text, name_en text, name_de text,
  name_fr text, name_it text, name_es text, name_nl text, name_ru text,
  brunnel text, intermittent int, tags hstore) AS $$
